    ...
    if (arguments.length == 0) {
        ...
        c.onload = function() {
            ...
            if (c.width * c.height > qrcode.maxImgSize) {
                ...
                f = Math.sqrt(qrcode.maxImgSize / i);
                j = i * f
            }
            h.width = j;
            h.height = f;
            ...
            try {
                qrcode.imagedata = g.getImageData(0, 0, h.width, h.height)
            } catch (k) {
...
function FinderPatternFinder() {
    ...
    this.selectBestPatterns = function() {
        ...
        if (h > 3) {
            ...
            var j = 0;
            for (var d = 0; d < h; d++) {
                var g = this.possibleCenters[d].EstimatedModuleSize;
                b += g;
                j += (g * g)
            }
            var a = b / h;
            this.possibleCenters.sort(function(m, l) {
                var k = Math.abs(l.EstimatedModuleSize - a);
                var i = Math.abs(m.EstimatedModuleSize - a);
                if (k < i) {
                    return (-1)
                } else {
                    if (k == i) {
            ...
            for (var d = 0; d < this.possibleCenters.length && this.possibleCenters.length > 3; d++) {
                var f = this.possibleCenters[d];
                if (Math.abs(f.EstimatedModuleSize - a) > c) {
                    this.possibleCenters.remove(d);
                    d--
                }
            }
        }
        if (this.possibleCenters.length > 3) {
            this.possibleCenters.sort(function(k, i) {
                if (k.count > i.count) {
                    return -1
                }
                if (k.count < i.count) {
    ...
    this.getNextBits = function(f) {
        ...
        if (f < this.bitPointer + 1) {
            ...
            if (f < this.bitPointer + 1 + 8) {
                var j = 0;
                for (var e = 0; e < this.bitPointer + 1; e++) {
                    j += (1 << e)
                }
                ...
                if (f < this.bitPointer + 1 + 16) {
                    ...
                    this.blockPointer++;
                    for (var e = 0; e < f - (this.bitPointer + 1 + 8); e++) {
                        h += (1 << e)
                    }
